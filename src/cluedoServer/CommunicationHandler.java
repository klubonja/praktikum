	...
	private void serverLogic(String message) {
		...
		if (checker.isValid()) {
			if (checker.getType().equals("accuse")) {
				...
				JoinGameStatus status = dataGuiManager.joinGame(gameID, color,
						client);
				if (status == JoinGameStatus.added) {
					dataManager.notifyAll(NetworkMessages.player_addedMsg(
							NetworkMessages.player_info(client.getNick(),
				...
				int gameID = checker.getMessage().getInt("gameID");
				dataManager.notifyAll(NetworkMessages.dice_resultMsg(gameID,
						dataManager.getGameByID(gameID).rollTheDice()));
			}

			else if (checker.getType().equals("move")) {
				int xKoordinate = checker.getMessage().getJSONObject("field")
						.getInt("x");
				int yKoordinate = checker.getMessage().getJSONObject("field")
						.getInt("y");
				CluedoField field = new CluedoField(new CluedoPosition(
				...
				dataManager.notifyAll(NetworkMessages.movedMsg(gameID,
						personpos));
				...
				// TODO notifyNextRound() methode auf CluedoGameServer
				// implementieren
				// setNextRound();
				...
				if (chatmsg.has("gameID")) {
					...
					dataGuiManager.addMsgIn(chatmsg.getString("timestamp")
							+ " " + chatmsg.getString("sender")
							+ " says (privately) : \n"
							+ chatmsg.getString("message"));
					dataManager.getClientByNick(chatmsg.getString("nick"))
							.sendMsg(NetworkMessages.chatMsg(msg, ts));
				} else {
					dataManager.notifyAll(NetworkMessages.chatMsg(msg, ts));
					dataGuiManager.addMsgIn(ts + " "
							+ chatmsg.getString("sender") + " says : \n" + msg);
				}
			} else {
