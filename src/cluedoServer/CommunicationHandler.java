	...
	private void awaitingLoginAttempt() {
		...
		while (!readyForCommunication) { // will keep listening for valid login
			...
			try {
				...
				NetworkHandhakeCodes errcode = checker.validateExpectedType(
						"login", null);

				if (errcode == NetworkHandhakeCodes.OK) {
					client.setExpansions(auxx.makeConjunction(
							Config.EXPANSIONS, checker.getMessage()
									.getJSONArray("expansions")));
					...
					if (dataGuiManager.addNetworkActor(client, "logged in"))
						dataManager.notifyAll(NetworkMessages
								.user_addedMsg(client.getNick()));
					else {
						client.sendMsg(NetworkMessages.error_Msg(client
								.getNick()
				...
				auxx.loginfo("login attempt :"
						+ checker.getMessage().toString());
			} catch (Exception e) {
				auxx.logsevere(
		...
		while (run) {
			try {
				...
				if (checker.isValid()) {
					if (checker.getType().equals("accuse")) {
					...
					} else if (checker.getType().equals("create game")) { // CREATE
																			// GAME
						createGame(checker.getMessage().getString("color"),
								client);
					} else if (checker.getType().equals("join game")) {
						int gameID = checker.getMessage().getInt("gameID"); // JOIN
																			// GAME
						String color = checker.getMessage().getString("color");
						JoinGameStatus status = dataGuiManager.joinGame(gameID,
								color, client);
						if (status == JoinGameStatus.added) {
							dataManager
									.notifyAll(NetworkMessages.player_addedMsg(
											NetworkMessages.player_info(client
													.getNick(), color,
													PlayerStates.do_nothing
															.getName()), gameID));
							auxx.logfine(status.name());
						} else if (status == JoinGameStatus.already_joined)
							client.sendMsg(NetworkMessages
									.error_Msg("you have already joined this game"));
						else if (status == JoinGameStatus.nick_already_taken)
							client.sendMsg(NetworkMessages
									.error_Msg("color is already chosen by someone else"));
						else if (status == JoinGameStatus.not_joinable)
							client.sendMsg(NetworkMessages
									.error_Msg("you can't join this game, idiot"));
					} else if (checker.getType().equals("start game")) { // START
																			// GAME
						int gameID = checker.getMessage().getInt("gameID");
						if (dataManager.getGameByID(gameID).hasNick(
								client.getNick())) {
							...
							dataGuiManager.startGameByID(gameID,
									client.getNick());
							dataGuiManager.addMsgIn("game "
							...
							client.sendMsg(NetworkMessages
									.error_Msg("you cant start this game"));
						}
					} else if (checker.getType().equals("leave game")) { // CREATE
																			// GAME
						dataGuiManager.removePlayerfromGame(client, checker
						...
						if (chatmsg.has("gameID")) {
							...
							dataManager.getGameByID(gameID).notifyAll(
									NetworkMessages.chatMsg(msg, gameID, ts));
						} else if (checker.getMessage().has("nick")) {
							dataGuiManager.addMsgIn(chatmsg
									.getString("timestamp")
									+ " "
									+ chatmsg.getString("sender")
									+ " says (privately) : \n"
									+ chatmsg.getString("message"));
							dataManager.getClientByNick(
									chatmsg.getString("nick")).sendMsg(
									NetworkMessages.chatMsg(msg, ts));
						} else {
							dataManager.notifyAll(NetworkMessages.chatMsg(msg,
									ts));
							dataGuiManager.addMsgIn(ts + " "
									+ chatmsg.getString("sender")
									+ " says : \n" + msg);
						}
					} else {
					...
					client.sendMsg(NetworkMessages.error_Msg(checker
							.getErrString()
							+ " \n "
							+ "bye "
							+ client.getNick()
							+ " and "
							+ client.getGroupName() + " is a shitty group"));
					client.sendMsg(NetworkMessages.disconnectMsg());
